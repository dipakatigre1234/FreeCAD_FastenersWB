...
def makeWingNut(self, fa):
    """Wing nut generator — DIN 315, ASME B18.6.9A, ASME B18.6.9.3, ASME B18.6.9.2, ASME B18.6.9.4(A/B), and ASME B18.6.9.6."""
    SType   = fa.baseType
    is_asme = SType.startswith("ASME")
    dia     = self.getDia(fa.calc_diam, True)

    # ── Hardened curve-type check (FreeCAD raises TypeError on some edges) ─────
    ...
    # ── Safe bounded fillet (prevents OCC runaway/hang on bezier edges) ────────
    ...
    if SType == "DIN315":
        ...
        D = avg(D_max, D_min); E = avg(E_max, E_min); F = avg(F_max, F_min); G = avg(G_max, G_min)
        d2 = D; d3 = E; m = G; g = C; h = B; e = A
        wing_r = g / 5
    ...
    # ── Topology Helpers ──────────────────────────────────────────────────────
    def make_line(p1, p2): return Part.LineSegment(p1, p2).toShape()
    def make_bezier(pts):
    ...
    if SType == "ASMEB18.6.9.6":
        # === TYPE C STYLE 3 - FLAWLESS PARAMETRIC TEARDROP ===
        Z_cyl = G * 0.15         
        ...
        try:
            ...
            if circ_edges: boss = boss.makeFillet(0.015, circ_edges)
        except Exception: pass
        shape = boss

        # 2. FULLY FRACTIONAL PARAMETRIC ENGINE 
        rA = A / 2.0; rD = D / 2.0; rE = E / 2.0
        W = rA - rD           # Available Wing Width
        ...
        v3 = Base.Vector(rD - ov_top, 0.0, G)
        ...
        def _fallback_wing():
            # Primitive box that securely intersects the boss
            box = Part.makeBox(W + ov_top, C, B - G)
            box.translate(Base.Vector(rD - ov_top, -C/2.0, G))
            return box
        ...
        wing = wing_base.copy() if wing_base else None

        # 3. RECESS TRACK (Perfect Proportion Teardrop)
        recess_face = None
        try:
        ...
        # 4. RECESS CUTTING
        if recess_face is not None and wing is not None:
            try:
                rec_depth = C / 4.0
        ...
        # 6. ASSEMBLE WITH CASCADING FALLBACKS
        def safe_fuse(base_solid, wing_solid):
            if wing_solid is None or not wing_solid.isValid() or wing_solid.Volume <= 1e-9:
                return None
            try:
                res = base_solid.fuse(wing_solid)
                if res.isValid() and res.Volume > base_solid.Volume:
                    return res
            except Exception:
                pass
            return None
        ...
        # Try fusing highest detail shape to lowest detail shape
        for candidate in [wing, wing_base, _fallback_wing()]:
            trial_fuse = safe_fuse(shape, candidate)
            if trial_fuse is not None:
                shape = trial_fuse
                successful_wing = candidate
                break
                
        # Generate the opposite wing using the successful geometry
        if successful_wing is not None:
            wing_left = successful_wing.copy()
            wing_left.rotate(Base.Vector(0,0,0), Base.Vector(0,0,1), 180)
            trial_fuse = safe_fuse(shape, wing_left)
            if trial_fuse is not None:
                shape = trial_fuse
        ...
        # === TYPE C STYLE 1 - FLAWLESS PARAMETRIC SPLINE ===
        Z_cyl = G * 0.15         
        Z_chamf = G * 0.20       
        ...
        if edges_to_fillet:
            try: wing = wing.makeFillet(fillet_rad, edges_to_fillet)
            except Exception: pass
        ...
        for e in shape.Edges:
            try:
                ...
                if (rD * 0.7) < dist < (rF * 1.3):
                    is_simple = _curve_is(e, (Part.Circle, Part.Line))
                    if not is_simple and (Z_cyl * 0.5) < cm.z < (G * 1.5):
                        root_edges.append(e)
            except Exception:
                pass
        if root_edges:
            try: shape = shape.makeFillet(root_rad, root_edges)
            except Exception: pass
            
        try: shape = shape.removeSplitter()
        except Exception: pass

    elif SType == "ASMEB18.6.9.3":
        # === CUSTOM FLAWLESS TANGENT SWEEP (Type B / Style 3) ===
        approx_inner_rad = dia / 2.0
        ...
        for e in wing.Edges:
            ...
            for v in e.Vertexes:
                ...
                if pt.z < fillet_z - 0.05 or pt.z > G + 0.05:
                    on_mating_face = False; break
                dz = pt.z - fillet_z
                dz_total = G - fillet_z
                expected_x = X_bot + dz * (X_top - X_bot) / dz_total if dz_total > 0 else X_bot
                if abs(pt.x - expected_x) > 0.05:
                    on_mating_face = False; break
            if not on_mating_face: edges_to_fillet.append(e)

        try:
            if edges_to_fillet: wing = wing.makeFillet(wing_r, edges_to_fillet)
        except Exception: pass
        ...
        # === CUSTOM DUAL TANGENT SWEEP (Type A / Style 2) ===
        approx_inner_rad = dia / 2.0
        overlap = max(0.5, (min(D/2.0, E/2.0) - approx_inner_rad) * 0.45)
        ...
        # --- AESTHETIC PROPORTIONAL MATH FOR TYPE A ---
        r_tip = G / 2.0
        r_bot = B * 0.8   
        r_top = A * 0.6   
        ...
        def get_tangent_arc_data(p_fixed, r_arc, center_tip, r_tip_val, maximize='z'):
            ...
            ca = (c_A if c_A.z > c_B.z else c_B) if maximize=='z' else (c_A if c_A.x > c_B.x else c_B)
            ...
            v1 = p_fixed - ca
            if v1.Length == 0: v1 = Base.Vector(1,0,0)
            v1.normalize()
            v2 = p_tan - ca
            if v2.Length == 0: v2 = Base.Vector(1,0,0)
            v2.normalize()
    ...
    body_wall_dia = min(d2, d3) if SType in ["ASMEB18.6.9.3", "ASMEB18.6.9.2", "ASMEB18.6.9.4A", "ASMEB18.6.9.4B", "ASMEB18.6.9.4", "ASMEB18.6.9.6"] else d2
