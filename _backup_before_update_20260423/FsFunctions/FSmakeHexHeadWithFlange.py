...
FIX: Thread was cutting into the flange body.
     Root cause — original code used:
         tl_cut   = b
         offset_z = -(length - b)
     When b is large, offset_z approaches 0 (flange face), letting
     the thread cutter enter the flange/fillet zone.

     Fix — mirrors makeHexHeadBolt logic exactly:
         tl_cut   = min(b, max(length - r1, 0.0))
         offset_z = -(length - tl_cut)
     This hard-clamps the thread top to z = -r1 (bottom of fillet arc),
     no matter how long b is.

  z =  0          <- flange bottom face         <- thread must NOT reach here
  z = -r1         <- fillet arc bottom           <- thread TOP stops HERE
  z = offset_z    <- thread start (near tip)
  z = -length     <- bolt tip
"""
...
def makeHexHeadWithFlange(self, fa):
    ...
    fm.AddArc2(r1, 0.0, -90)                         # fillet arc (r1 zone)
    ...
    #      tl_cut   = b
    #      offset_z = -(length - b)
    #  When b >= length, offset_z reaches 0 → thread cuts into flange! ✗
    #
    #  FIX:
    #      tl_cut   = min(b, max(length - r1, 0.0))
    #                       └── hard ceiling: distance from tip to fillet bottom
    #      offset_z = -(length - tl_cut)
    #                         └── thread TOP always lands at z = -r1 ✓
    #
    if fa.Thread:
        tl_cut   = min(b, max(length - r1, 0.0))   # FIXED: stops thread at z = -r1
        offset_z = -(length - tl_cut)
        if is_asme:
