...
def cut_thread(shape, fa, dia, tl, offset_z, P_mm=None):
    ...
    _pct   = _interpolated_deviation_pct(dia) if (_d_raw and _d_raw > 0) else 0.0
    _dev   = (_d_raw * _pct / 100.0) if (_d_raw and _d_raw > 0) else 0.0
    FreeCAD.Console.PrintMessage(
        f"[ASME cut] nom={nominal}  series={series}  tpi={tpi}  cls={cls}\n"
        f"  P                      = {P_mm:.4f} mm\n"
        f"  Thread_Outer_Dia (CSV) = {_d_raw:.5f} mm\n"
