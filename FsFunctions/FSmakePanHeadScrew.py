...
def makePanHeadScrew(self, fa):
    ...
    # ── ASMEB18.6.3.9A – slotted pan head ────────────────────────────────────
        ...
        # CSV cols (mm) - same notation as 10Adef but R instead of O (pan head has crown radius R):
        # P, A_max, A_min, H_max, H_min, R, J_max, J_min, T_max, T_min
        P, A_max, A_min, H_max, H_min, R_head, \
    ...
    # ── ASMEB18.6.3.9B – cross-recessed pan head ──────────────────────────────
        ...
        # CSV cols (mm) - same notation as 10Bdef but R instead of O:
        # P, A_max, A_min, H_max, H_min, R, M_ref, Driver, p_max, p_min
        P, A_max, A_min, H_max, H_min, R_head, \
        ...
        intersect_angle_rad = math.acos(min((dk_max / 2 - R_head) / (rf - R_head), 1.0))
    ...
    # ── ASMEB18.6.3.9C – hexalobular pan head ─────────────────────────────────
        ...
        # CSV cols (mm): P, A_max, A_min, H_max, H_min, R, DriveSize, p_max, p_min
        # Old CSV also had A_ref between DriveSize and p_max — handle both layouts.
        if len(fa.dimTable) == 10:
            # Old layout: P, A_max, A_min, H_max, H_min, R, DriveSize, A_ref, p_max, p_min
            P, A_max, A_min, H_max, H_min, R_head, DriveSize, _A_ref, p_max, p_min = fa.dimTable
        else:
            # New layout: P, A_max, A_min, H_max, H_min, R, DriveSize, p_max, p_min
            P, A_max, A_min, H_max, H_min, R_head, DriveSize, p_max, p_min = fa.dimTable
        dk_max = (A_max + A_min) / 2
        k_torx = (H_max + H_min) / 2
        rf     = R_head if R_head > dk_max / 2 else dk_max
        ...
        recess.translate(Base.Vector(0.0, 0.0, k_torx))
        if getattr(fa, "SecurityPin", False):
            security_pin_solid = _make_security_pin_cylinder(tt, 0.0, _fs_float(k_torx), standard="ASME", pin_dia_mm=getattr(fa, "SecurityPinDiameter", 0.0))

    # ── ASMEB18.6.3.10A – slotted oval-head ──────────────────────────────────
    elif SType == "ASMEB18.6.3.10A":
        # CSV cols (mm): P, A_max, A_min, H_max, H_min, O_max, O_min, J_max, J_min, T_max, T_min
    ...
    # ── ASMEB18.6.3.10B – cross-recessed oval-head ────────────────────────────
        ...
        # CSV cols (mm): P, A_max, A_min, H_max, H_min, O_max, O_min,
    ...
    # ── ASMEB18.6.3.10C – hexalobular oval-head ───────────────────────────────
        ...
        # CSV cols (mm): P, A_max, A_min, H_max, H_min, O_max, O_min, DriveSize, p_max, p_min
        ...
        recess.translate(Base.Vector(0.0, 0.0, k))
        if getattr(fa, "SecurityPin", False):
            security_pin_solid = _make_security_pin_cylinder(tt, 0.0, _fs_float(k), standard="ASME", pin_dia_mm=getattr(fa, "SecurityPinDiameter", 0.0))

    # ── ASMEB18.6.3.12A – slotted truss head ─────────────────────────────────
    elif SType == "ASMEB18.6.3.12A":
        # CSV cols (mm) - same notation as 10Adef but R instead of O (truss head):
        # P, A_max, A_min, H_max, H_min, R, J_max, J_min, T_max, T_min
    ...
    # ── ASMEB18.6.3.12B – cross recessed truss head ───────────────────────────
        ...
        # CSV cols (mm) - same notation as 10Bdef but R instead of O:
        # P, A_max, A_min, H_max, H_min, R, M_ref, Driver, p_max, p_min
        P, A_max, A_min, H_max, H_min, R, M_ref, Driver, p_max, p_min = fa.dimTable
        P      = P * 25.4
        dk_max = (A_max + A_min) / 2
        k      = (H_max + H_min) / 2
        rf     = R
    ...
    if SType == "ASMEB18.6.3.9A":
        ...
        fm.AddArc2(-R_head * math.cos(intersect_angle_rad),
                   ...
                   "ASMEB18.6.3.10C", "ASMEB18.6.3.9C"):
